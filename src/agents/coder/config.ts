...
// ciWaiterGuard and fileOpGuard (the pure Bash-command classifiers these hooks
// wrap) live in ./bash-guard.ts — general-purpose command parsing with nothing
// agent-specific about it, extracted so it's independently unit-testable
// (see tests/agents/coder/bash-guard.test.ts).
  ...
  return {
    ...
    buildPrompt(state: PipelineState, ctx: PipelineContext): string {
      ...
      const parts = [
        ...
        `1. **Trigger only:** \`bun /app/scripts/await-pipeline.ts --branch <your-branch> --trigger-only\` (pipeline ${config.azureDevOps.ciPipelineId}). It triggers the build, prints \`runId=<id>\`, and exits 0 immediately (it does NOT print a runnable poll command). Capture \`<id>\` — your authoritative CI run id; report it as \`ciRunId\`.`,
        ...
        `- **Parse existing results:** \`bun /app/scripts/parse-mcp.ts <file> errors\` for previously saved pipeline timeline files.`,
        ``,
        `## Instructions`,
