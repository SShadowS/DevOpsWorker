...
describe('stats.ts SQL shape', () => {
  ...
  test('every pr_reviews query in the population-aware endpoints filters on is_test', () => {
    ...
    // shared totalN/otherPopulationCount helper) + 4 (review-value: the
    // `EXISTS (... FROM pr_reviews ...)` population predicate on each of the
    // findings and other-population queries, the spend query, and the
    // findings_list "raised" count — that endpoint's SUBJECT is
    // finding_outcomes, but it still reaches pr_reviews for population
    // scoping, for cost, and for what review actually raised, and each of
    // those reaches is exactly what this guard exists to police) = 16.
