...
echo ""
echo "Dashboard:      https://<DASHBOARD_HOSTNAME, or dashboard.localhost if unset — see .env>/"
echo "Webhook server: http://$(hostname -I | awk '{print $1}'):3001"
echo ""
echo "Verify:"
echo "  docker compose logs -f --tail 50    # Watch logs"
echo "  curl -s http://localhost:3001/health   # webhook server only — the dashboard is behind Caddy's TLS now, not a plain localhost port"
echo ""
